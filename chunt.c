...
void output(char* filename, int lineNum) {
	...
	switch (g_outputmode) {
		case MODE_TEXT:
			printf("%s/%s:%d:%s\n", g_basedir, filename, lineNum, g_buffer);
			break;

		case MODE_HTML:
			// we need to replace '<' and '>' with html entities to avoid them breaking the html output
			snprintf(g_trash, 65535, "<tr><td>%s/%s</td><td><a href=\"chunt://%s/%s:%d\">%d</a></td><td>", g_basedir, filename, g_basedir, filename, lineNum, lineNum);
			jdx = strlen(g_trash);
...
void processFile(char* filename) {
	...
	int ch, idx = 0;
	int lineNum = 1;
	int bStarted = 0, bInQuotes = 0, bInComment = 0;
